  ...
  methods: {
    ...
    ReDraw() {
      ...
      if (this.data.series && Object.keys(this.data.series).length > 0) {
        ...
        //  对 类目轴 areaStyle 的 color 的设置
        if (this.chartDirection == 'left') {
          this.option.yAxis.splitArea.areaStyle.color = splitAreaColor;
        } else {
          this.option.xAxis.splitArea.areaStyle.color = splitAreaColor;
        }
        // 对 data的series 进行设置
        for (let i = 0; i < this.data.series.length; i++) {
          ...
          if (this.data.series[i].type != 'null' && this.data.series[i].type != null) {
            series.push({
              name: this.data.series[i].name,
              label: {
                show: this.data.series[i].showlable == undefined && this.data.series[i].data.length < 36 ? true : this.data.series[i].showlable,
